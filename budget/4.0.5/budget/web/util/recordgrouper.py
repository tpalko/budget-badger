...
import json
from web.models import RecordGroup, Record
...
class RecordGrouper(object):
    ...
    def group_records(force_regroup_all=False):
        ...
        if force_regroup_all:
            ...
            Record.objects.all().update(record_group=None)
        
        records = Record.objects.filter(record_group__isnull=True)
        ...
        for description in set([ r.description or '' for r in records ]):
            ...
            record_group = None 
            record_group_matches = RecordGroup.objects.filter(name=description)

            if len(record_group_matches) > 0:
                record_group = record_group_matches[0]
            else:
